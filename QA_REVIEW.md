...
## 2026-09-15T18:51:55.410Z — 1/1 Normalize advisories and separate from trip delays

### Visual advisories (auto-accepted)
- requirement coverage: 0 satisfied, 2 uncertain, 0 unmet — Both requirements are 'uncertain'. The diff plausibly implements advisory classification at ingestion (is_non_trip_advisory -> status 'advisory'), in the Supabase mapping (normalizeApiStatus), and in the feed's fallback styling (STATUS_STYLE.advisory), but none of the four specific alert IDs are referenced and the STATUS_STYLE definitions showing the actual 'Advisory'/'Notice' badge text are not in the diff. The second requirement targets metrics-dashboard counter/average logic whose file does not appear in FILES TOUCHED or CHANGE SET at all, so there is no proof of absence — only absence of proof. No requirement can be marked 'missing' because the diff contains no line that contradicts or stubs the required behavior.
- QA concern: None of the four named alert IDs appear in the diff, so the requirement is tied to a general classification rule rather than verified per-alert behavior.
- QA concern: The STATUS_STYLE object in IncidentFeed.jsx is not shown; the diff assumes a STATUS_STYLE.advisory key exists, but its presence and its rendered label ('Advisory'/'Notice' vs something else) are not evidenced.
- QA concern: The metrics dashboard that would 'not increment trip delay counters or skew delay duration averages' is not part of this diff, so the counting/averaging behavior is untouched and unverified.
- QA concern: map_alert() in scripts/ingest_go_api.py still derives delay_minutes via to_int(...) and the diff does not show it being nulled or suppressed for advisories, so the value stored for advisories is not visible.
- QA concern: src/components/IncidentFeed.jsx now routes every unrecognized status to advisory styling, which could mask genuine delays whose status value is not a known key; this is a behavioural consequence that can only be assessed at runtime.
- web browser visual QA: The screenshot shows the dashboard's default TODAY view with a trip-delay incident timeline (+6 MIN DELAY entries), tracked trips, weekly pattern, and recent days panels. None of the specified advisory alerts (M0000524722, M0000524728, M0000526118, M0000526567) are visible, and no 'Advisory'/'Notice' badge appears anywhere to confirm correct classification. The metric tiles (Total Alerts 40 vs Trips Affected 4, Avg Delay 11m) cannot be decomposed to show whether advisory items with null delay minutes and empty trip lists are excluded from trip delay counts and duration averages. The requirement is therefore not observable in this view rather than visibly violated.
- Screenshot: C:\Users\brian\Projects\slowgo-frontend\.agent\screenshots\web-1789498309266.png
